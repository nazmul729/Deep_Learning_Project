...
model = Sequential()
# The LSTM layer
model.add(LSTM(units = number_of_units, activation = activation_function, input_shape=(None, 1)))
#model.add(Dropout(0.2))
model.add(Dense(units = 1)) # Add output layer
#model.add(Activation('relu'))
model.compile(optimizer = optimizer_name, loss = loss_function, metrics=['mse', 'accuracy'])
